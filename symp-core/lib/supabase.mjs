...
// ── Profile-foundation v2 providers ─────────────────────────────────────────
// Entities / events / tombstones / settings / synthesis / fact-history. The v2
// CONSUMERS (knowledge-extractor v2, knowledge-context, blak-profile-synthesis,
// symp-v1-listener) shipped to main importing these, but this provider layer
// never landed — breaking EVERY Netlify function build (astro build doesn't
// bundle functions, so it passed locally while prod failed). Restored to match
// supabase/migrations/20260621_profile_foundation.sql.
...
export async function fetchEntities(userId) {
    ...
    const { ok, data } = await sbFetch(
        ...
        `&select=id,kind,name,role,relation,notes,sentiment,importance,birthday,visibility,source,confidence,status`
    );
    return (ok && Array.isArray(data)) ? data : [];
}

export async function upsertEntity({ userId, kind, name, role = null, relation = null, notes = null, sentiment = null, source = 'blak', confidence = 0.6 }) {
    if (!userId || !kind || !name) return { ok: false, action: 'noop' };
    const u = encodeURIComponent(userId), k = encodeURIComponent(kind), n = encodeURIComponent(name);
    const nowIso = new Date().toISOString();
    const existing = await sbFetch(`symp_knowledge_entities?user_id=eq.${u}&kind=eq.${k}&name=eq.${n}&select=id,status`);
    const row = (existing.ok && Array.isArray(existing.data) && existing.data[0]) ? existing.data[0] : null;
    if (row) {
        if (row.status === 'user_edited') return { ok: true, action: 'locked-skip' };
        const body = { last_seen_at: nowIso, updated_at: nowIso };
        if (role != null) body.role = role;
        if (relation != null) body.relation = relation;
        if (notes != null) body.notes = notes;
        if (sentiment != null) body.sentiment = sentiment;
        if (source) body.source = source;
        if (Number.isFinite(confidence)) body.confidence = confidence;
    ...
    const ins = await sbFetch('symp_knowledge_entities', {
        method: 'POST',
        body: { user_id: userId, kind, name, role, relation, notes, sentiment, source, confidence },
        prefer: 'return=minimal',
    });
    return { ok: ins.ok, action: 'inserted' };
}
...
export async function fetchEvents(userId) {
    ...
    const { ok, data } = await sbFetch(
        ...
        `&order=occurred_on.asc.nullslast` +
        `&select=id,title,kind,occurred_on,occurred_at,end_on,all_day,recurrence,notes,source,confidence,status`
    );
    return (ok && Array.isArray(data)) ? data : [];
}

export async function upsertEvent({ userId, title, kind = 'event', occurredOn = null, occurredAt = null, endOn = null, allDay = true, recurrence = null, entityId = null, factId = null, notes = null, source = 'blak', sourceKind = null, sourceRef = null, confidence = 0.6 }) {
    if (!userId || !title) return { ok: false, action: 'noop' };
    // No natural unique key on events — avoid dupes by matching title (+ date).
    const u = encodeURIComponent(userId), t = encodeURIComponent(title);
    let q = `symp_knowledge_events?user_id=eq.${u}&title=eq.${t}&select=id`;
    if (occurredOn) q += `&occurred_on=eq.${encodeURIComponent(occurredOn)}`;
    const existing = await sbFetch(q);
    if (existing.ok && Array.isArray(existing.data) && existing.data[0]) return { ok: true, action: 'exists' };
    const ins = await sbFetch('symp_knowledge_events', {
        method: 'POST',
        body: { user_id: userId, title, kind, occurred_on: occurredOn, occurred_at: occurredAt, end_on: endOn, all_day: allDay, recurrence, entity_id: entityId, fact_id: factId, notes, source, source_kind: sourceKind, source_ref: sourceRef, confidence },
        prefer: 'return=minimal',
    });
    return { ok: ins.ok, action: 'inserted' };
}
...
export async function addTombstone({ userId, scope = 'fact', area = null, key = null, valueNorm = null, reason = null }) {
    ...
    const res = await sbFetch('symp_knowledge_tombstones?on_conflict=user_id,area,key,value_norm', {
        method: 'POST',
...
export async function fetchProfileSettings(userId) {
    ...
    const { ok, data } = await sbFetch(`symp_profile_settings?user_id=eq.${encodeURIComponent(userId)}&select=*`);
    return (ok && Array.isArray(data) && data[0]) ? data[0] : null;
}
...
export async function fetchRecentMoodSummary(userId, days = 14) {
    ...
    const { ok, data } = await sbFetch(
        `mood_logs?user_id=eq.${encodeURIComponent(userId)}&created_at=gte.${encodeURIComponent(since)}&select=mood&order=created_at.desc&limit=60`
    );
    if (!ok || !Array.isArray(data) || !data.length) return null;
    const counts = {};
    for (const r of data) if (r.mood) counts[r.mood] = (counts[r.mood] || 0) + 1;
    let dominant = null, max = 0;
    for (const m in counts) if (counts[m] > max) { max = counts[m]; dominant = m; }
    return { n: data.length, dominant };
}

export async function saveSynthesis({ userId, narrative, factCount = 0, model = null }) {
    if (!userId) return { ok: false };
    const res = await sbFetch('symp_profile_synthesis?on_conflict=user_id', {
        method: 'POST',
        body: { user_id: userId, narrative: narrative || null, fact_count: factCount, model, generated_at: new Date().toISOString() },
        prefer: 'resolution=merge-duplicates,return=minimal',
    });
    return { ok: res.ok };
}
...
export async function fetchFactHistory(userId, factId) {
    if (!userId || !factId) return [];
    const { ok, data } = await sbFetch(
        `symp_knowledge_fact_history?user_id=eq.${encodeURIComponent(userId)}&fact_id=eq.${encodeURIComponent(factId)}&order=changed_at.desc&select=old_value,new_value,changed_by,reason,changed_at`
    );
    return (ok && Array.isArray(data)) ? data : [];
}

export async function recordFactHistory({ userId, factId, area, key, oldValue, newValue, changedBy = 'blak', reason = null }) {
    if (!userId || !factId) return { ok: false };
    const res = await sbFetch('symp_knowledge_fact_history', {
        method: 'POST',
